...
<!-- 			</div> -->
			<table class="table table-striped table-bordered table-hover">
				...
				<tr>
					...
					<th style="text-align:left"><a href="coverlettercontent.do?num=${article.num}" OnClick="location.href='coverlettercontent.do?num=${article.num}'">${article.subject} [${article.commCount}]</a></th>
				...
				<c:if test="${count > 0}">
   				...
   				<c:if test="${numPageGroup > 1}">
        			<a href="./coverletterlist.do?pageNum=${(numPageGroup-2)*pageGroupSize+1 }">[이전]</a>
   				</c:if>

   				<c:forEach var="i" begin="${startPage}" end="${endPage}">
       				<a href="./coverletterlist.do?pageNum=${i}">[
        			<font color="#000000">
   				...
   				<c:if test="${numPageGroup < pageGroupCount}">
        			<a href="./coverletterlist.do?pageNum=${numPageGroup*pageGroupSize+1}">[다음]</a>
   				</c:if>
				</c:if>		
				</div>
				
					<div style="text-align:right">
						<input type=button class="btn btn-success" value="작성하기" OnClick="window.location='coverletterWriteform.jsp'">
					</div>
